...
WHERE 
    ...
    AND mode_paiement.type != 'cash' AND patients.pat_id=? 
GROUP BY 
    patients.pat_id, 
    patients.pat_nom, 
    patients.pat_postnom, 
    ...
    if (isset($_SESSION['ad_id'])) {
		  ...
		  if(isset($_POST['paiement_dette']))
        {
         ...
         if(isset($_POST['paiement_dette_tous']))
        {
            ...
            //sql to insert captured values
    $query="UPDATE paiement_service SET id_mode_paiement = ? WHERE pat_id = ?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('ss', $id_mode_paiement, $id_patient);
            $stmt->execute();   
            /*
            *Use Sweet Alerts Instead Of This Fucked Up Javascript Alerts
            *echo"<script>alert('Successfully Created Account Proceed To Log In ');</script>";
                                ...
                                <div class="card">
                                    ...
                                    <h4>Noms : <?php echo $rowp->pat_prenom;?>
                                     ...
                                     Sexe :  <?php echo $rowp->pat_sexe;?><br>
                                        ...
                                        <div>Total :  <strong><b style="color:red"> <?php echo $rowp->total_usd;  ?> $ et <?php  echo $rowp->total_fc; ?> fc</b></strong>  
        </div> 

                          <form method="post">

<input type="hidden" required="required" name="id_patient" class="form-control" id="inputEmail4" placeholder="" value="<?php echo $rowp->id_patient;?>">
                      
                    <button type="submit" name="paiement_dette_tous" class="ladda-button btn btn-primary" data-style="expand-right">Payer toutes les Dettes</button>

                  <a href="gestion-dettes.php" class="ladda-button btn btn-warning" data-style="expand-right">Retour</a>
...
<input type="hidden" required="required" name="id_paiement_service" class="form-control" id="inputEmail4" placeholder="" value="<?php echo $row->id_paiement_service;?>">
       <div>Dette de <strong><b style="color:red"> <?php  echo $row->prix; ?> <?php  echo $row->type_money; ?></b></strong> du Service <?php  echo $row->description; ?> en date du <?php  echo $row->date_update; ?>   </div>        
                                      
                                <button type="submit" name="paiement_dette" class="ladda-button btn btn-primary" data-style="expand-right">Payer</button>
